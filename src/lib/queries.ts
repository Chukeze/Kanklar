...
import { z } from "zod"
import { CreateMediaType, CreateFunnelFormSchema, UpsertFunnelPage } from "./types"
...
export const GetErrorMessageHelper = async(message:string) => {
...
export const saveActivityLogsNotification = async ({
    ...
    if (!userData) {
        return GetErrorMessageHelper('Could Not Find User')
    }
    ...
    if (!foundAgencyId){
        if (!subaccountId){
            return GetErrorMessageHelper('No Agency Found and No Subaccount Found, you need to provide at least a agency id or subaccount id')
        }
...
export const initUser = async (newUser: Partial<User>) => {
    ...
    await clerkClient.users.updateUserMetadata(user.id, {
        privateMetadata: { role: newUser.role || 'USER'},
    })
